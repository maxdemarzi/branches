...
(defn mergeNds [nds mgLevel] (let [mgd (mergeAll nds mgLevel)] (prt "merged" (count mgd) "from" (count nds)) mgd))
...
(defn trainDStream
      ([X rMap mgLevel]
        (let [root (atom nil)]
             (trainDStream rMap [[X [root] (labelAr X) false]] 1 0 (.getColumns X) 0 Double/MAX_VALUE 1 mgLevel)
             (deref root)))
      ([rMap inNodes impurity bedRes featN failsN bestImpurity cycle mgLevel]
        (if (not-empty (remove last inNodes))
          (let [nodesInf (mergeNds inNodes mgLevel)
                          ...
                          (mapcat
                            (fn [[X pAtoms Y]]
                                (let [splt (bestSplit X featN)]
                                     (if (and (not (empty? splt)) (< failsN (Math/sqrt (.getRows X))) (> mgLevel (first splt)))
               ...
               (recur rMap nds (min newImpurity impurity) (if (< newImpurity impurity) 0 (+ 1 bedRes)) featN
                      (if (< newImpurity bestImpurity) 0 (+ 1 failsN)) (min newImpurity bestImpurity) (+ 1 cycle) mgLevel)))))
...
;(defn dStream []
;      (let [fullData (.transpose (org.jblas.DoubleMatrix. (into-array (map double-array (mapv cons trainY trainX)))))
